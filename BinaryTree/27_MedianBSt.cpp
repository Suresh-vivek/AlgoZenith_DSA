...
float findMedian(struct Node* root) {
    ...
    inorder.clear();  // Clear the inorder vector to ensure no residual values from previous runs.
    inord(root);
    ...
    if (len & 1) {
        ...
        return (inorder[len / 2 - 1] + inorder[len / 2]) / 2.0;  // Use floating-point division.
    }
}
